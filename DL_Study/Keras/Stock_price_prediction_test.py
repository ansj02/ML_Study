'''
import pandas_datareader.data as web
import datetime

start = datetime.datetime(2018, 12, 19)
end = datetime.datetime(2020, 3, 1)

gs = web.DataReader("078930.KS", "yahoo", start, end)

print(gs.index)
print(gs.columns)

print(gs['Close']-gs['Adj Close'])

print(gs['Close']['2020-02-10'])
print(gs['Close'][0])

print(len(gs['Close']))

Change_Value=[]

for i in range(len(gs['Close'])) :
    Change_Value.append(gs['Close'][i] - gs['Close'][i-1])

print(Change_Value)
'''

from keras.models import Sequential
from keras.layers import Dense, LSTM
import numpy as np
import pandas_datareader.data as web
import datetime

start = datetime.datetime(2010, 12, 19)
end = datetime.datetime(2020, 3, 1)

gs = web.DataReader("035720.KS", "yahoo", start, end)

Change_Value=[]
n=4 # n-1일동안 주식변동치로 n번째일의 주식변동치 예측

for i in range(len(gs['Close'])) :
    Change_Value.append(gs['Close'][i] - gs['Close'][i-1])

while(len(Change_Value)%n != 0) :
    del Change_Value[0]


ComparisonTemp=[]
inputdataTemp=[]

for i in range(len(Change_Value)) :
    if i%n == n-1 :
        ComparisonTemp.append(Change_Value[i])
    else :
        inputdataTemp.append(Change_Value[i])
n=n-1
inputdata = [inputdataTemp[i * n:(i + 1) * n] for i in range((len(inputdataTemp) + n - 1) // n )]
#Comparison = [ComparisonTemp[i * 1:(i + 1) * 1] for i in range((len(ComparisonTemp)) // 1 )]
Comparison = ComparisonTemp

x = np.array(inputdata)
y = np.array(Comparison)

print(x.shape)
print(y.shape)
x = x.reshape((x.shape[0], x.shape[1], 1))
print(x.shape)

from sklearn.model_selection import train_test_split
x_train, x_test, y_train, y_test = train_test_split(x, y, random_state=66, test_size=0.2)
x_test, x_val, y_test, y_val = train_test_split(x_test, y_test, random_state=66, test_size=0.5)


model = Sequential()
#ex)model.add(Dense(units=64, activation='relu', input_dim=100))
#model.add(Dense(100, activation='elu', input_dim=n))
model.add(LSTM(10, activation='elu', input_shape=(n,1), return_sequences=True))
model.add(LSTM(10, activation='elu', return_sequences=True))
model.add(LSTM(10, activation='elu', return_sequences=True))
model.add(LSTM(10, activation='elu', return_sequences=True))
model.add(LSTM(10, activation='elu', return_sequences=True))
model.add(LSTM(10, activation='elu', return_sequences=True))
model.add(LSTM(1))

'''
from tensorflow.keras.layers import Input, Dense
from tensorflow.keras.models import Model
inputs = Input(shape=(n,))
hidden1 = Dense(10, activation='elu')(inputs)
hidden2 = Dense(12, activation='elu')(hidden1)
hidden3 = Dense(8, activation='elu')(hidden2)
hidden4 = Dense(7, activation='elu')(hidden3)
hidden5 = Dense(5, activation='elu')(hidden4)
outputs = Dense(1, activation='elu')(hidden5)
model = Model(inputs = inputs, outputs = outputs)
'''

model.compile(loss='mae', optimizer='adam', metrics=['acc'])

from keras.callbacks import EarlyStopping
early_stopping = EarlyStopping(monitor = 'mae', patience = 10, mode ='max')

model.fit(x_train, y_train, epochs=100, batch_size=1, validation_data=(x_val, y_val), callbacks = [early_stopping])


loss, mae = model.evaluate(x_test, y_test, batch_size=1)
print('acc : ', mae)

y_predict = model.predict(x_test)
print(y_predict,y_test)


'''테스트값으로 하나 뽑아서 예측값과 비교
teststart = datetime.datetime(2019, 3, 1)
testend = datetime.datetime(2019, 5, 20)

testVTemp1 = web.DataReader("035720.KS", "yahoo", teststart, testend)

testVTemp2=[]

for i in range(len(testVTemp1['Close'])) :
    testVTemp2.append(testVTemp1['Close'][i] - testVTemp1['Close'][i-1])

while(len(testVTemp2) != n+1) :
    del testVTemp2[0]

answer = testVTemp2[n]
del testVTemp2[n]

testV = np.array([testVTemp2])
#print(testV, testVTemp2, answer)
y_predict = model.predict(testV)
#print('예측값:'+int(y_predict)+' 실제값:'+answer)
print('예측   실제')
print(y_predict, answer)
'''
#model.summary()